...
public class Solver {
	/** 用来放Memory的栈 */
	private Stack<Memory> stack = new Stack<Memory>();
	
	/** 要解的目标map */
	private SudokuMap map;

	/** 构造函数 */
	public Solver(SudokuMap map) {
		this.map = map;
	}
	...
	public void doSolve() {
		...
		if (map == null) {
			System.out.println("No map to be solved");
		}
		if (scanInitialMap()) {
			map.printResult();
	 ...
	 */
	private boolean scanInitialMap() {
		for (int i=0; i<9; i++) {
			for (int j=0; j<9; j++) {
	 ...
	 * @param map SudokuMap
	 */
	private void createMemoryAndPush(SudokuMap map) {
